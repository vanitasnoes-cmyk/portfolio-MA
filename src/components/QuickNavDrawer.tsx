...
export function QuickNavDrawer({
  ...
  const sectionIcon = (id: string) => {
    if (id === 'gioi-thieu') return <GraduationCap className="w-4 h-4 text-violet-500" />;
    if (id === 'du-an') return <FileText className="w-4 h-4 text-violet-500" />;
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700">
          <div className="flex items-center gap-2">
            ...
            <span className="font-extrabold text-blue-950 dark:text-violet-300 text-sm">Điều hướng nhanh</span>
          </div>
          <Button
            type="button"
            onClick={onClose}
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold text-left cursor-pointer ${
                    currentSection === link.id
                      ? 'bg-violet-50 dark:bg-violet-950/50 text-violet-700 dark:text-violet-300 border border-violet-100 dark:border-blue-800'
              ...
              {portfolioProjects.map((proj, idx) => (
                <button
                  ...
                  className={cn('text-left px-3 py-2.5 rounded-lg text-xs font-bold transition-colors cursor-pointer',
                    activeTab === idx
                      ? 'bg-violet-700 text-white shadow-xs'
                      : 'text-slate-600 dark:text-slate-300 hover:bg-violet-50 dark:hover:bg-violet-950/40'
                  )}
                >
            ...
            <section>
              ...
              <div className="flex flex-col gap-0.5 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                {stepTexts.map((text, idx) => (
                  ...
                  >
                    <span className="text-violet-500 font-black shrink-0 w-5 text-right leading-tight mt-px">{idx + 1}.</span>
